...
write_iframe_index <- function(root_dir, slug, title = NULL, author = NULL, date = NULL) {
  index_file <- file.path(root_dir, "index.qmd")

  # Build YAML header with optional metadata

  yaml_lines <- c("---")
  if (!is.null(title) && nchar(title) > 0) {
    yaml_lines <- c(yaml_lines, sprintf("title: %s", yaml_quote(title)))
  }

  if (!is.null(author) && nchar(author) > 0) {
    yaml_lines <- c(yaml_lines, sprintf("author: %s", yaml_quote(author)))
  }
  if (!is.null(date) && nchar(date) > 0) {
    yaml_lines <- c(yaml_lines, sprintf("date: %s", yaml_quote(date)))
  }
  yaml_lines <- c(
    yaml_lines,
    "page-layout: full",
...
create_pdf_index <- function(root_dir, slug) {
  ...
  # Try to extract metadata from .Rmd file
  rmd_file <- file.path(root_dir, paste0(slug, ".Rmd"))
  meta <- parse_front_matter(rmd_file)
